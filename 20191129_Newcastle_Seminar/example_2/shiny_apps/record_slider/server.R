...
## Data needed
plot_df = readRDS("/Users/katesaunders/Documents/Git/Talks/Older/completionSeminar/shiny_apps/record_slider/plot_df.rds")
# plot_df = plot_df %>%
  ...
  output$recordPlot <- renderPlot({
    ggplot() +
      ...
      geom_point(data = plot_df %>%
                 ...
                 aes(x= x, y =y, group = year), shape = 3, col = "mediumblue") +
      geom_point(data = plot_df %>%
                   filter(record_dry_year == input$year),
                 aes(x= x, y =y, group = year), shape = 4, col = "red") +
      coord_fixed() +
      xlab("Longitude") +
      ylab("Latitude") +
