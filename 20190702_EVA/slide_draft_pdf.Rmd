...
author: "Kate Saunders @katerobsau and Kiri Whan @kirianwhan"
date: "27/05/2019"
output: 
...
* Deal with the trend in changing temperature   
(Lazy: Assume stationarity in a given period)

* Wlog we can transform the marignal distributions to   
<font color="2292B5">$F_i(z) = \exp(-1/z)$</font> 

* We have common marginals
...
<font color="2292B5"> $$V_{ij}(z) = \left(\frac{1}{z_i} + \frac{1}{z_j}\right)A\left(\frac{z_i}{|z_i + z_j|} + \frac{z_j}{|z_i + z_j|}\right)$$ </font>
...
## Data Example: Logistic Copula

```{r, echo = FALSE, warning = FALSE, message = FALSE}
alpha = seq(0.05, 1, 0.05)
# 
# bvdata <- lapply(alpha, function(x){
#   bvdata <- rbvevd(500, dep = x, model = "log",
#                    mar1 = c(0,1,0),
#                    mar2 = c(0,1,0)) %>%
#     as.data.frame() %>%
#     mutate(alpha = rep(x, 500))
#   return(bvdata)
# }) %>% do.call("rbind", . )
...
pickands_plot <- ggplot() +
  geom_path(data = pickdata,
            aes(x = x, y = y, frame = alpha), col = "black") +
  geom_line(data = NULL, aes(x = c(0, 0.5, 1), y = c(1, 0.5, 1)), linetype = "dotted") +
  ylim(c(0, 1)) +
  coord_fixed() +
  ylab("A(t)") +
...
grViz("digraph flowchart {
      ...
      [5]: '...'
      [6]: 'Mhalla, L. Chavez-Demoulin, V. and Naveau,P. (2017): Nonlinear dependence'
      ")
```
...
pickands_plot <- ggplot() +
  geom_path(data = dep_early,
            aes(x = x, y = y), col = "black") +
  geom_line(data = rbind(dep_early[i, ], c(0.5,0)),
            aes(x = x, y = y), col = "black", 
